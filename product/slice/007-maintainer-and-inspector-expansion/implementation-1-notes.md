...
- **S1 — Autofix-cycle fixture redesign** — directory renamed (`testsuite/fixtures/maintainer/` → `testsuite/fixtures/autofix/`) via `git mv` preserving history; new 4-part fixture format (YAML front-matter with mandatory `inspector`/`finding`/`maintainer`/`resolution` symbol fields, three body documents: input / finding-slot plist / expected fixed code); `validate-one-autofix-cycle-fixture` replaces `validate-one-maintainer-fixture`.
...
|---|-----------|:------:|-------------|
| AC1 | `testsuite/fixtures/autofix/` replaces `testsuite/fixtures/maintainer/` | ✓ | `git mv` preserved history; `git status` shows `R` entries |
...
| AC9 | `validate-one-maintainer-fixture` no longer in source | ✓ | `Grep` returns zero hits across `src/`, `testsuite/` |
| AC10 | No regressions | ✓ | `atelier/testsuite:run-all-tests` → **357/357 passing** in clean SBCL 2.6.3 subprocess |
...
1. **Fixture count: plan said 10 existing fixtures, reality is 6.** The 4 missing maintainers (`fix-labels-to-flet`, `fix-header-line`, `fix-footer-line`, `fix-project-identification`) never had discoverable fixtures — they are covered by dedicated ad-hoc testcases in `testsuite/maintainers/`. Not a blocker, reduces migration workload.
...
3. **fix-mixed-indentation fixture removed entirely.** Per user directive: "discoverable fixtures are a convenience, not a goal." The fixture's expected output `  (defvar *x* 1)` had semantically meaningful leading whitespace that could never be a `read ⟫ pretty-print` fixed point, and the existing `testsuite/maintainers/fix-mixed-indentation.lisp` already provides full coverage via a synthetic finding. The fixture was removed via `git rm -f`; its directory is gone from both the source tree and the autofix-cycle discovery.
...
2. **Live-image ASDF reload pitfall.** Force-loading `org.melusina.atelier/testsuite` from a live SBCL image fails with a package-variance error because the image has accumulated dozens of exported test symbols from incremental `define-testcase` calls, and the new DEFPACKAGE would "un-export" them. Workaround during execution: run the full regression in a fresh SBCL subprocess via `sbcl --no-userinit --script`. **Suggested mitigation:** document as a memory entry alongside the existing live-image pitfalls, and consider using `sb-ext:*on-package-variance*` configuration or a helper function that unexports stale symbols before reload.
...
- **Directory rename via `git mv`** rather than copy + delete. Preserves file history so `git log -- testsuite/fixtures/autofix/fix-earmuffs/baseline.text` still shows the original slice 005/006 commits.
...
**New files (test):**
- `testsuite/inspectors/check-single-branch-if.lisp`
- `testsuite/inspectors/check-single-form-progn.lisp`
- `testsuite/inspectors/check-when-not.lisp`

**New files (fixtures):**
- `testsuite/fixtures/inspector/check-single-branch-if/{when,unless,implicit-nil,clean}.lisp`
- `testsuite/fixtures/inspector/check-single-form-progn/{baseline,clean,good}.lisp`
- `testsuite/fixtures/inspector/check-when-not/{baseline,clean}.lisp`
...
**Modified (test):**
- `testsuite/package.lisp` — import `assert-equal` from confidence
- `testsuite/utilities.lisp` — `autofix-cycle-fixture-error` condition, `read-autofix-cycle-fixture`, `discover-autofix-cycle-fixtures`, `autofix-cycle-fixture` accessor, `:autofix` kind on `fixture`; legacy `maintainer-fixture`, `discover-maintainer-fixtures`, `read-maintainer-fixture` and `:maintainer` kind removed
- `testsuite/autofix.lisp` — `autofix-cycle-finding-accessor`, `autofix-cycle-finding-slot-check`, `run-autofix-cycle`, `autofix-cycle-comparison-level`, `autofix-cycle-results-equal-p`, `validate-one-autofix-cycle-fixture`, `validate-autofix-cycle-fixtures`, `validate-autofix-cycle-fixtures-as-pretty-printer-fixed-points`; legacy `validate-one-maintainer-fixture` and `validate-maintainer-fixtures` removed; `testsuite-autofix` entry updated

**Modified (fixtures — migrated from 2-part to 4-part format):**
- `testsuite/fixtures/autofix/fix-earmuffs/baseline.text`
- `testsuite/fixtures/autofix/fix-constant-naming/baseline.text`
- `testsuite/fixtures/autofix/fix-bare-lambda/baseline.text`
- `testsuite/fixtures/autofix/fix-bare-loop-keywords/baseline.text` (expected rewritten to pretty-printer canonical multi-line)
- `testsuite/fixtures/autofix/fix-trailing-whitespace/baseline.text`

**Removed (per user directive):**
- `testsuite/fixtures/autofix/fix-mixed-indentation/` (entire directory)

**Moved (as-is, silently skipped by new loader):**
- All 20 `testsuite/fixtures/autofix/fix-line-too-long/*.text` files
