...
| I50 | Every autofix-cycle fixture whose **inspector operates at the :SYNTAX level** has its expected fixed code document asserted as a pretty-printer fixed point: `(string= x (pretty-print-form (read-from-string x) 0))` holds for every such document. This means the pretty-printer is the single authority on canonical text for Lisp forms — text-resolution maintainers at the syntax level must emit what the pretty-printer would produce, not a textual variant. Line-inspector fixtures are excluded because their expected output is whitespace-significant text, not a form. Maintainers that do not fit the discoverable-fixture format (e.g. `fix-mixed-indentation`) use ad-hoc `define-testcase` tests instead. |
