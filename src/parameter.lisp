...
It is an error to sort BINDINGS featuring a cyclic dependency."
  (let ((graph
    (flet ((map-item (binding)
             (cons (car binding) (list-parameter-names (cdr binding)))))
      (mapcar #'map-item bindings))))
    (labels
  ((next (path visited node)
     ...
     (edges
       ...
       (flet ((reduce-item (visited node)
                (explore nextpath visited node)))
         (reduce #'reduce-item edges :initial-value visited))))
   ...
   (toposort ()
     (flet ((reduce-item (visited binding)
              (dfs visited (car binding))))
       (reduce #'reduce-item bindings :initial-value nil))))
      (toposort))))
