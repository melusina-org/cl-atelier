...
(defconstant +maximum-nesting-depth+ 16
...
(defvar *default-configuration*
  (list :name "default"
        ...
        :entries nil
        :metadata (list :created-by "system"
                        :created-at "2026-01-01"
                        :priority 0
                        :tags (list :builtin :readonly)
...
(defparameter *configuration-search-path*
  (list #P"/etc/atelier/" #P"/usr/local/etc/atelier/" (merge-pathnames #P".config/atelier/" (user-homedir-pathname))
        #P"./")
...
(deftype configuration-key () "A keyword symbol used as a configuration entry key." '(and keyword (not null)))


(defstruct (configuration-entry
            (:constructor make-configuration-entry (&key key value documentation source priority))
            (:predicate configuration-entry-p)
            (:copier nil))
...
(defclass configuration-manager nil
  ((name
    :initarg :name
    :reader configuration-manager-name
    :type string
    :documentation "Human-readable name for this configuration manager.")
   (entries
    :initarg :entries
    :accessor configuration-manager-entries
    :type hash-table
    :initform (make-hash-table :test 'eq)
    :documentation "Hash table mapping configuration keys to their entries.")
   (parent
    :initarg :parent
    :reader configuration-manager-parent
    :type (or configuration-manager null)
    :initform nil
    :documentation "Optional parent manager for hierarchical lookup.")
   (read-only-p
    :initarg :read-only-p
    :reader configuration-manager-read-only-p
    :type boolean
    :initform nil
    :documentation "When true, mutation operations signal an error.")
   (change-hooks
    :initarg :change-hooks
    :accessor configuration-manager-change-hooks
    :type list
    :initform nil
    :documentation "List of functions called with (key old-value new-value)
...
(defgeneric initialize-manager (manager source)
...
(defmethod initialize-manager ((manager configuration-manager) (source pathname))
...
(defmethod initialize-manager ((manager configuration-manager) (source list))
  ...
  (loop :for (key value) :on source :by #'cddr
        :do (setf (gethash key (configuration-manager-entries manager))
                  (make-configuration-entry :key key
                                            :value value
                                            :source "plist")))
  manager)


(defun make-configuration-manager (&rest initargs &key name entries parent read-only-p change-hooks)
...
The literals \"true\" and \"false\" become T and NIL."
  (declare (type string raw-value)
           (values t))
  (cond ((string-equal raw-value "true")
         t)
        ((string-equal raw-value "false")
         nil)
        ((every #'digit-char-p raw-value)
         (parse-integer raw-value))
        (t
         raw-value)))
...
(defun create-configuration-report
    (manager &key (stream *standard-output*)
                  (format :text)
                  (include-defaults t)
                  (include-documentation nil)
                  (sort-by :key)
                  (filter-fn nil)
                  (max-entries most-positive-fixnum)
...
TITLE is the report heading."
  (declare (type configuration-manager manager)
           ...
           (type string title)
           (values null))
  (format stream "~&~A~%~A~%" title (make-string (length title) :initial-element #\=))
  (let* ((entries
         ...
         (sorted
           (sort (copy-list entries) #'string<
                 :key (ecase sort-by
                        (:key
                         (lambda (e)
                           (symbol-name (configuration-entry-key e))))
                        (:priority
                         (lambda (e)
                           (princ-to-string (configuration-entry-priority e))))
                        (:source
                         #'configuration-entry-source))))
         (limited
...
Returns (VALUES NIL NIL) if KEY is not found anywhere in the chain."
  ...
  (labels ((walk-chain (current-manager depth)
             ...
             (let ((entry
                     (gethash key (configuration-manager-entries current-manager))))
               (if entry
...
Returns T if valid, NIL otherwise."
  (declare (type configuration-entry entry)
           (values boolean))
  (let* ((key
           (configuration-entry-key entry))
         (value
           (configuration-entry-value entry))
         (valid-key-p
           (and (keywordp key) (not (null key))))
         (valid-value-p
           (not (null value))))
...
(defun collect-active-entries (manager &key (include-defaults t) (filter-fn nil))
...
only entries satisfying the predicate are included."
  ...
  (let ((result
    ...
    (labels ((collect-from-manager (current-manager)
               ...
               (when (and include-defaults (configuration-manager-parent current-manager))
                 (collect-from-manager (configuration-manager-parent current-manager)))))
    ...
    (:text
     ...
     (when include-documentation
       (unless (string= "" (configuration-entry-documentation entry))
         (format stream "    ~A~%" (configuration-entry-documentation entry)))))
    (:sexp
     (format stream "~&  (~S ~S)"
             (configuration-entry-key entry)
             (configuration-entry-value entry)))
    (:table
     (format stream "~&| ~20A | ~30S | ~10A |"
             (configuration-entry-key entry)
             (configuration-entry-value entry)
...
pairs.  Returns the number of defaults applied."
  ...
  (when (configuration-manager-read-only-p manager)
    (error "Cannot apply defaults: manager ~S is read-only." (configuration-manager-name manager)))
  (let ((count
          0)
        (entries
          (configuration-manager-entries manager)))
    ...
    (case target-type
      ...
      (:boolean
       (case value
         ...
         (t
          (if (stringp value)
              (not (member value '("false" "no" "0" "nil") :test #'string-equal))
              (not (null value))))))
      (t
       (error "Unknown target type ~S for coercion of ~S." target-type value)))))


(defun update-configuration-entry (manager key &key value documentation priority)
  "Update an existing configuration entry in MANAGER, modifying only
the slots for which arguments are provided.  Returns the updated entry."
  (declare (type configuration-manager manager)
           (type configuration-key key)
           (values configuration-entry))
  (let ((entry
          (gethash key (configuration-manager-entries manager))))
    (unless entry
      (error "No configuration entry for key ~S." key))
    (setf (configuration-entry-value entry) (or value (configuration-entry-value entry))
          (configuration-entry-documentation entry) (or documentation (configuration-entry-documentation entry))
          (configuration-entry-priority entry) (or priority (configuration-entry-priority entry)))
    entry))


(defun walk-configuration-tree (manager function &key (max-depth +maximum-nesting-depth+))
  "Walk the configuration manager parent chain, calling FUNCTION on
each manager.  FUNCTION receives two arguments: the manager and the
current depth (0 for the root).  Stops at MAX-DEPTH."
  (declare (type configuration-manager manager)
           (type function function)
           (type fixnum max-depth)
           (values null))
  (labels ((walk (current-manager depth)
             (funcall function current-manager depth)
             (let ((parent
                     (configuration-manager-parent current-manager)))
               (when (and parent (< depth max-depth))
                 (walk parent (1+ depth))))))
    (walk manager 0))
  nil)


(defun format-configuration-summary (manager &key (stream *standard-output*) (include-parents t))
  "Print a one-line summary for each entry in MANAGER to STREAM.
Uses FLET with keyword-parameter lambda lists for internal formatting."
  (declare (type configuration-manager manager)
           (type stream stream)
           (type boolean include-parents)
           (values null))
  (flet ((format-entry-line (entry &key (prefix "") (show-source t))
           (format stream "~&~A~A = ~S"
                   prefix
                   (configuration-entry-key entry)
                   (configuration-entry-value entry))
           (when show-source
             (format stream " [~A]" (configuration-entry-source entry)))
           (terpri stream)))
    (maphash (lambda (key entry)
               (declare (ignore key))
               (format-entry-line entry :prefix "  "
                                        :show-source include-parents))
             (configuration-manager-entries manager)))
  nil)
...
TITLE is the overall report heading."
  ...
  (flet ((format-manager-section (manager)
           (with-output-to-string (section-stream)
             (format section-stream "~&--- ~A ---~%" (configuration-manager-name manager))
    ...
    (let ((separator
      ...
      (concatenate 'string
        title
        separator
        (reduce (lambda (accumulated next)
                  (concatenate 'string
                    accumulated
                    separator
                    next))
...
(defun merge-configurations (primary secondary &key (conflict-resolution :primary-wins))
...
Returns a new CONFIGURATION-MANAGER."
  (declare (type configuration-manager primary secondary)
           (type (member :primary-wins :secondary-wins :higher-priority) conflict-resolution)
           (values configuration-manager))
  (let ((merged
          (make-configuration-manager :name (concatenate 'string
                                              (configuration-manager-name primary)
                                              "+" (configuration-manager-name secondary)))))
    (flet ((merge-entry (key primary-entry secondary-entry)
             (cond ((null secondary-entry)
                    primary-entry)
                   ((null primary-entry)
                    secondary-entry)
                   (t
                    (ecase conflict-resolution
                      ...
                      (:higher-priority
                       (if (>= (configuration-entry-priority primary-entry)
                           ...
                           secondary-entry)))))))
      (let ((entries
              (configuration-manager-entries merged)))
        (maphash (lambda (key entry)
...
HANDLER receives two arguments: the condition and the pathname."
  ...
  (let ((manager
          (make-configuration-manager :name (namestring pathname))))
    (handler-case (progn (initialize-manager manager pathname) manager)
      (file-error (condition)
        (funcall handler condition pathname)
        nil)
...
(defun find-configuration-in-search-path
    (&key (search-path *configuration-search-path*) (filename "config.sexp") (on-error :skip))
    ...
    (let ((candidate
      ...
      (when (probe-file candidate)
        (restart-case (let ((manager
                              (make-configuration-manager :name (namestring candidate))))
                        (initialize-manager manager candidate)
                        (return manager))
          (skip-configuration-file nil
