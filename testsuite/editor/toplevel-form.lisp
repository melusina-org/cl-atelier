...
(define-testcase validate-read-preserves-feature-conditional ()
  ...
  (let* ((input "(defun connect () #+sbcl :yes #-sbcl :no)")
    ...
    ;; The raw s-expression includes only the matching branch
    (assert-equal 'common-lisp:defun (atelier/editor:toplevel-form-kind form))
    ;; The source-text preserves the full input
...
(define-testcase validate-ast-overridden-features ()
  ...
  (let* ((form (atelier/editor:read-toplevel-form-from-string
         ...
         ;; With empty features, #+sbcl should not match
         ;; But cst:raw already has the matching branch baked in,
         ;; because the reader evaluated #+sbcl at parse time.
         ;; The AST with different features still returns cst:raw.
         (ast (atelier/editor:toplevel-form-ast form :features '(:ccl))))
    ;; The AST is derived from cst:raw which was read under the current
    ;; platform's features. This is the documented lossy behavior.
    (assert-t* (consp ast))))
