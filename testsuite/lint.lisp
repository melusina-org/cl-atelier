...
(define-testcase validate-lint-contents ()
  (flet ((linter1 (contents)
	 ...
	 (linter2 (contents)
	   (declare (ignore contents))
	   (let ((atelier/legacy::*hint-code* :hint-linter-2))
	     (atelier/legacy:hint-at-file "Linter 2")))
	 (linter3 (contents)
	   (declare (ignore contents))
	   (list
	    (let ((atelier/legacy::*hint-code* :hint-linter-3-1))
	      (atelier/legacy:hint-at-file "Linter 3 1"))
	    (let ((atelier/legacy::*hint-code* :hint-linter-3-2))
	      (atelier/legacy:hint-at-file "Linter 3 2")))))
    (multiple-value-bind (hints contents)
	(atelier/legacy::lint-contents "Some file contents" (list #'linter1 #'linter2 #'linter3))
      (assert-string= "A fixed content" contents)
      (assert-eq 3 (length hints))
      (assert-type hints 'list)
      (loop :for item :in hints
	    :do (assert-type item 'atelier/legacy::hint)))))
...
(define-testcase validate-lint-lines ()
  (flet ((linter1 (line)
	 ...
	 (linter2 (line)
	   (when (< 10 (length line))
	     (let ((atelier/legacy::*hint-code* :hint-linter-2))
	       (atelier/legacy:hint-at-file-line "Linter 2")))))
    (multiple-value-bind (hints lines)
	(atelier/legacy::lint-lines
	 ...
	 (list #'linter1 #'linter2))
      ...
      (assert-eq 2 (slot-value (first hints) 'atelier/legacy::line))
