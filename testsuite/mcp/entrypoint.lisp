...
(define-testcase run-mcp-tests ()
  ...
  ;; Debugger tests (slow, spawns one child)
  (run-debugger-tests)
  ;; Debugger tool count (fast)
  (validate-debugger-tool-count)
  ;; ASDF/Quicklisp/Confidence tools (slow, spawns one child)
  (run-asdf-tools-tests)
  ;; ASDF tool count (fast)
  (validate-asdf-tool-count)
  ;; Documentation tools (slow, spawns one child)
  (run-documentation-tools-tests)
  ;; HyperSpec tests (slow, filesystem)
